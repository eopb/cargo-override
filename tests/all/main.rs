...
fn sub_dependency_is_of_source_git(git_url: &str) {
    ...
    let sub_dependency_name = "sub_dependency";
    let sub_dependency_folder = sub_dependency_name.to_string();
    let sub_dependency_folder_path = working_dir.join(sub_dependency_folder.clone());
    ...
    let _patch_manifest_path = create_cargo_manifest(
        &patch_folder_path,
        &Manifest::new(Header::basic(patch_crate_name).version("1.2.0".to_owned()))
            .add_target(Target::lib(patch_crate_name, "src/lib.rs"))
            .render(),
    );

    // setup local sub dependency
    fs::create_dir(&sub_dependency_folder_path).expect("failed to create patch folder");
    let _sub_dependency_manifest_path = create_cargo_manifest(
        &sub_dependency_folder_path,
        &Manifest::new(Header::basic(sub_dependency_name).version("1.1.5".to_owned()))
            .add_target(Target::lib(sub_dependency_name, "src/lib.rs"))
            .add_dependency(Dependency::new(patch_crate_name, "1.0").git(git_url))
            .render(),
    );
    ...
    let manifest = Manifest::new(Header::basic(package_name))
        .add_target(Target::bin(package_name, "src/main.rs"))
        .add_dependency(Dependency::new(sub_dependency_name, "1.1.5").path("sub_dependency"))
        .render();
    let working_dir_manifest_path = create_cargo_manifest(working_dir, &manifest);
    ...
    insta::allow_duplicates! {
        insta::with_settings!({
            ...
            [dependencies]
            sub_dependency = { version = "1.1.5", path = "sub_dependency" }
